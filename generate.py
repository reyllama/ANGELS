...
def interpolate(args, g_ema, device, mean_latent):
    with torch.no_grad():
        ...
        for i in tqdm(range(args.pics)):
            ...
            sample_w = torch.cat([torch.lerp(w1, w2, v) for v in torch.linspace(0, 1, args.sample, device=device)], dim=0)
            sample, _ = g_ema(
            ...
            utils.save_image(
                sample,
                args.root+f"/sample/interpolationW/{str(i+1).zfill(6)}.png",
                nrow=args.sample,
                normalize=True,
                range=(-1,1)
            )
...
def generate(args, g_ema, device, mean_latent):
    ...
    with torch.no_grad():
        g_ema.eval()
        dirichlet = torch.distributions.dirichlet.Dirichlet(torch.ones(8)/8)
        # a = None
        for i in tqdm(range(args.pics)):
            sample_z = torch.randn(args.sample, args.latent, device=device)
            alpha = dirichlet.sample((args.sample, ))
            alpha[0] = torch.ones(args.sample)/8
            # print(alpha)
