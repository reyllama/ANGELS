...
def train(args, loader, generator, discriminator, extra, g_optim, d_optim, e_optim, g_ema, device):
    ...
    # k = 1
    # d_ratio_mean = 0
    lowp, highp = 0, args.highp     # TODO: We need args.highp

    # equidistance_reg = True
    # adaptive = False
    # evaluate_interp = True

    dirichlet = torch.distributions.dirichlet.Dirichlet(torch.ones(args.batch)/args.dir_div, validate_args=None)
    ...
    for idx in pbar:
        ...
        if which > 0:
            alpha = dirichlet.sample((args.batch,)).to(device)  # (Batch, Batch)
        ...
        if which > 0:
            ...
            w = generator.get_latent(z) # (Batch, 512)
            sample_w = torch.matmul(alpha, w) # (Batch, 51)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir_div", type=int, default=1)
    parser.add_argument(
